...
export default function AdminPanel() {
  ...
  const [settings, setSettings] = useState({ legalName: "ИП Мусаев Жаныбек Кочкорбаевич", qualityControl: "Отдел контроля качества", telegram: "https://t.me/BIG_REST_TEAM" });
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setSelectedOrder(null); };
  ...
  const cancelOrder = async (order: Order) => {
    if (!window.confirm(`Отменить заказ №${order.orderNumber}? Это действие изменит статус заказа и исключит его из выручки.`)) return;
    try { await updateStatus(order, "cancelled"); setSelectedOrder(null); }
    catch (cancelError) { setError(cancelError instanceof Error ? cancelError.message : "Не удалось отменить заказ"); }
  };
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><img src="/assets/icons/logo.svg" alt="ДААНА СУШИ" /><nav>{menu.map(([id, label]) => <button className={tab === id ? "active" : ""} key={id} onClick={() => setTab(id)}>{label}</button>)}</nav><Link href="/catalog/1">Открыть сайт</Link><button className="logout" onClick={() => { localStorage.removeItem("sushi-admin-token"); setToken(""); }}>Выйти</button></aside><main className="admin-main"><header><div><h1>{menu.find(([id]) => id === tab)?.[1]}</h1><p>Управление «ДААНА СУШИ»</p></div><button onClick={() => void loadAll()}>Обновить</button></header>{error && <div className="admin-error banner">{error}</div>}{loading && <div className="admin-loading">Загрузка…</div>}
    ...
    {selectedOrder && <div className="admin-modal-backdrop"><button className="admin-modal-dismiss" type="button" onClick={() => setSelectedOrder(null)} aria-label="Закрыть подробности заказа" /><section className="admin-order-modal" role="dialog" aria-modal="true" aria-labelledby="admin-order-title"><header><div><span>Заказ</span><h2 id="admin-order-title">#{selectedOrder.orderNumber}</h2></div><button type="button" onClick={() => setSelectedOrder(null)} aria-label="Закрыть">×</button></header><div className="admin-order-status"><span className={`status-badge status-${selectedOrder.status}`}>{statusLabels[selectedOrder.status] || selectedOrder.status}</span><time>{new Date(selectedOrder.createdAt).toLocaleString("ru-RU")}</time></div><div className="admin-order-details"><div><small>Клиент</small><strong>{selectedOrder.customerName}</strong><a href={`tel:${selectedOrder.customerPhone.replace(/\D/g, "")}`}>{selectedOrder.customerPhone}</a></div><div><small>Точка самовывоза</small><strong>{selectedOrder.locationName}</strong><span>{selectedOrder.locationAddress}</span></div><div className="admin-order-comment"><small>Комментарий и время</small><span>{selectedOrder.comment || "Без комментария"}</span></div></div><div className="admin-order-lines"><h3>Состав заказа</h3>{selectedOrder.items.map((item, index) => <div key={`${item.productName}-${index}`}><span><b>{item.productName}</b><small>{item.unitPrice} С × {item.quantity}</small></span><strong>{item.lineTotal} С</strong></div>)}</div><footer><div><span>Итого</span><strong>{selectedOrder.total} С</strong></div><button type="button" className="danger" disabled={selectedOrder.status === "cancelled" || selectedOrder.status === "completed"} onClick={() => void cancelOrder(selectedOrder)}>{selectedOrder.status === "cancelled" ? "Заказ отменён" : "Отменить заказ"}</button></footer></section></div>}
  </main></div>;
}
