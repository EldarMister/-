...
export default function RewardsWithdrawalDialog({ coins, coinNetwork, nfts, onClose, onSubmit }: Props) {
  ...
  return (
    <div className="reward-withdrawal-overlay" role="dialog" aria-modal="true" aria-labelledby="reward-withdrawal-title">
      ...
      <section className="reward-withdrawal-dialog">
        <button className="reward-withdrawal-close" type="button" aria-label="Закрыть" onClick={close} disabled={submitting}><MaterialIcon>close</MaterialIcon></button>

        {step === "scanner" && <div className="reward-withdrawal-scanner">
          <header><button type="button" aria-label="Вернуться к форме" onClick={() => setStep("form")}><MaterialIcon>arrow_back</MaterialIcon></button><h2 id="reward-withdrawal-title">Сканирование QR</h2><span /></header>
        ...
        {step === "success" && <div className="reward-withdrawal-success">
          <span><MaterialIcon>check</MaterialIcon></span>
        ...
        {step === "confirm" && <div className="reward-withdrawal-content">
          <header className="reward-withdrawal-heading"><button type="button" aria-label="Изменить данные" onClick={() => { setError(""); setStep("form"); }}><MaterialIcon>arrow_back</MaterialIcon></button><span><h2 id="reward-withdrawal-title">Подтверждение вывода</h2><p>Проверьте данные перед отправкой заявки</p></span></header>
          ...
          <div className="reward-withdrawal-warning"><span><MaterialIcon>warning_amber</MaterialIcon></span><p>Проверьте адрес кошелька внимательно. После отправки заявки изменить его нельзя.</p></div>
        ...
        {step === "form" && <div className="reward-withdrawal-content">
          ...
          <div className="reward-withdrawal-kinds">
            <button type="button" className={`reward-kind-card coin${kind === "coins" ? " selected" : ""}`} disabled={coins <= 0} aria-pressed={kind === "coins"} onClick={() => chooseKind("coins")}><span>NAKTA Coin</span><strong>{new Intl.NumberFormat("ru-RU").format(coins)}</strong><small>Сеть: {networkLabels[coinNetwork as NftNetwork] || coinNetwork}</small><i>NC</i></button>
            <button type="button" className={`reward-kind-card nft${kind === "nft" ? " selected" : ""}`} disabled={!availableNfts.length} aria-pressed={kind === "nft"} onClick={() => chooseKind("nft")}><span>NFT</span><strong>{availableNfts.length}</strong><i><MaterialIcon>hexagon</MaterialIcon></i></button>
          </div>
