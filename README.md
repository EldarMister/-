...
Для временного входа одного доверенного номера без SMS можно задать серверную переменную `OTP_BYPASS_PHONE`. Номер не должен попадать в клиентский код; любой человек, который его знает, сможет войти в этот аккаунт, поэтому после использования переменную следует удалить.

`PORT`, `CORS_ORIGIN`, `NEXT_PUBLIC_API_URL` и email администратора на Railway указывать не нужно. В production CORS по умолчанию выключен, потому что сайт и API работают на одном origin; `CORS_ORIGIN` понадобится только при размещении браузерного frontend на другом HTTPS-домене. Ключ JavaScript API загружается в браузере и должен быть добавлен до команды Build; ограничьте его по HTTP Referer доменами `daanasushi.com`, `www.daanasushi.com` и доменом Railway для предварительной проверки. `YANDEX_GEOCODER_API_KEY` используется только сервером для поиска адреса и определения адреса по метке — не добавляйте к его имени префикс `NEXT_PUBLIC_`. При первом запуске схема и исходный каталог создаются автоматически. После изменения ключей выполните новый Deploy.
